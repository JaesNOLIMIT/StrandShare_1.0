-- Keep staff rejection limited to initial intake review and return a workflow-specific
-- message when a stale client tries to reject an application already sent to admin.
...
begin
  ...
  if lower(replace(replace(replace(coalesce(v_application."Status", ''), '_', ''), ' ', ''), '-', ''))
    <> 'pendingstaffreview'
    or coalesce(v_application."Linked_Event_Request_ID", 0) > 0
  then
    raise exception 'This application has already left staff review and can no longer be rejected by staff. If admin rejected it, use Submit Appeal to Admin.';
  end if;
