-- 105_align_bundle_waybill_only_schema.sql
-- Final alignment to bundle-waybill-only model:
-- - Hair_Submission_Bundles uses Bundle_Waybill_Code (no Submission_Code).
-- - Bundle RPCs no longer read/write Submission_Code.
-- - Wig code trigger reads Bundle_Waybill_Code.
...
-- Backfill bundle waybill from old Submission_Code when legacy column still exists.
do $$
begin
  if exists (
    select 1
    ...
    execute '
      ...
      set "Bundle_Waybill_Code" = coalesce(nullif(trim("Bundle_Waybill_Code"), ''''), nullif(trim("Submission_Code"), ''''))
      where coalesce(trim("Bundle_Waybill_Code"), '''') = ''''
        and coalesce(trim("Submission_Code"), '''') <> ''''
    ';
  end if;
end
$$;
...
-- Keep Wig_Code synced from Bundle_Waybill_Code.
...
begin
  if new."Bundle_ID" is not null then
    select b."Bundle_Waybill_Code" into bundle_code
    from public."Hair_Submission_Bundles" b
...
where w."Bundle_ID" = b."Bundle_ID"
  and coalesce(trim(w."Wig_Code"), '') = ''
  and coalesce(trim(b."Bundle_Waybill_Code"), '') <> '';

drop index if exists public."idx_Hair_Submission_Bundles_Submission_Code_unique";
