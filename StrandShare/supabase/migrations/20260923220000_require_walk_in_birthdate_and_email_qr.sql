...
-- Registered/account submissions retain the AI-screening guard. A public
-- event walk-in has no account or screening; assigned Staff performs the
-- complete manual hair check after scanning the attendee's QR waybill.
...
-- Keep p_age in the signature for deployed-client compatibility, but derive
-- the persisted age from the required birthdate on the server.
