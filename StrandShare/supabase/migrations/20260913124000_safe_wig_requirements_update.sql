...
-- Update the global requirements row through a deliberately scoped function.
-- This remains compatible with databases that enable the safe-update guard,
-- which rejects direct REST UPDATE statements it cannot prove are filtered.
create or replace function public.update_wig_requirements(
...
begin
  ...
  -- Use a singleton upsert instead of a standalone UPDATE. This works even when
  -- the database-wide safe-update guard rejects UPDATE statements at parse time.
...
revoke all on function public.update_wig_requirements(integer, numeric, boolean, boolean, boolean, boolean, text, text)
  from public, anon;
grant execute on function public.update_wig_requirements(integer, numeric, boolean, boolean, boolean, boolean, text, text)
  to authenticated;

comment on function public.update_wig_requirements(integer, numeric, boolean, boolean, boolean, boolean, text, text) is
  'Safely updates the global wig requirements singleton for active staff and administrators.';
