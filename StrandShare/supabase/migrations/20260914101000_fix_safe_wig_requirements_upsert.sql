...
-- Replace the standalone UPDATE used by the requirements RPC with a singleton
-- upsert. Some hosted databases enable a safe-update guard that rejects UPDATE
-- commands before their WHERE clause can be evaluated.
...
comment on function public.update_wig_requirements(integer, numeric, boolean, boolean, boolean, boolean, text, text) is
  'Safely upserts the global wig requirements singleton for active staff and administrators.';
