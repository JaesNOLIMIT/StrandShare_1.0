...
-- Repair databases where the lifecycle trigger was already installed against
-- the normalized attendee schema. Attendee identity now comes from users and
-- user_details through Event_Attendees.User_ID.
...
comment on function public.enqueue_program_lifecycle_notifications() is
  'Queues one-time Program Ended notices and Successful certificate emails using normalized attendee identities.';
