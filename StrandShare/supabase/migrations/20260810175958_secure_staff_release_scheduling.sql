...
create or replace function public.enforce_release_schedule_minimum_date()
returns trigger
language plpgsql
set search_path = ''
as $fn$
begin
  if new."Proposed_Release_Date"::date < (timezone('Asia/Manila', now())::date + 3) then
...
drop trigger if exists trg_enforce_release_schedule_minimum_date on public."Release_Schedules";
create trigger trg_enforce_release_schedule_minimum_date
before insert or update of "Proposed_Release_Date" on public."Release_Schedules"
for each row execute function public.enforce_release_schedule_minimum_date();
...
begin
  ...
  insert into public."Release_Schedules" (
    "Req_ID", "Hospital_ID", "Proposed_Release_Date", "Proposed_By", "Proposal_Note",
    "Hospital_Decision", "Is_Current", "Created_At", "Updated_At"
  ) values (
    p_req_id, v_request."Hospital_ID", v_local_release_date, v_actor.user_id,
  ...
  set "Status" = 'To Be Release',
      "Release_Date" = v_local_release_date,
      "Release_Requested_By" = v_actor.user_id,
      "Release_Requested_At" = timezone('Asia/Manila', now()),
      "Status_Reason" = null,
      "Updated_At" = timezone('Asia/Manila', now())
  where "Req_ID" = p_req_id
