-- 123_remove_submission_code_from_staff_rsvp_scan.sql
-- Remove Hair_Submissions.Submission_Code dependency from staff RSVP scan RPC.

begin;

drop function if exists public.scan_event_attendee_rsvp(integer, text);

create function public.scan_event_attendee_rsvp(
...
begin
  ...
  -- Primary match: direct attendee link.
  select count(*)
  into v_matches
  ...
  if v_matches > 1 then
    raise exception 'Multiple hair submissions linked to attendee_id=%.', v_attendee."Event_Attendee_ID";
  elsif v_matches = 1 then
    select *
    into v_submission
    from public."Hair_Submissions" hs
  ...
  set
    "Event_Attendee_ID" = coalesce("Event_Attendee_ID", v_attendee."Event_Attendee_ID"),
